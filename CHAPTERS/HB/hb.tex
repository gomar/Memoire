...
$E$ can thus be named the Fourier operator.
Note that conversely, using the inverse Fourier operator $E^{-1}$:
\begin{equation}
	\begin{split}
...
Note that the matrix matrix formulation does not require the
orthogonality of the complex exponentials.
Now multiplying the equation by the inverse Fourier operator $E^{-1}$:
\begin{equation}
...
we get $2N+1$ steady equations in the time
domain that are coupled by a source term.
The source term appears finally as a spectral operator defined as:
\begin{equation}
	D_t = i \omega E^{-1} K E.
...
a generalized Fourier operator defined as:
\begin{equation}
	(E)_{k,n} = \frac{1}{2N+1} e^{-i \omega_{k-N} t_n}.
\end{equation}
Note that replacing $\omega_{k-N}$ by $(k - N) \omega$ gives
the mono-frequential inverse Fourier operator back. 
This can be justified mathematically: in the
framework of the almost-periodic functions~\cite{Besicovitch1932},
one can approximated such a function (which is composed of several
frequencies non necessarily harmonically related) by an 
...
\begin{equation}
	...
	e^{i \omega_k t}.
\end{equation}
This allows the use of the multi-frequential Fourier operator as defined
above. However, in the multi-frequential case, the inverse Fourier operator
...
compute its derivative and inverse numerically $E^{-1}$ to obtain
$E$.

Using the same development as for the mono-frequential formulation,
...
Finally, the multi-frequential formulation of 
Eq.~\ref{eq:sm_nonlinear_convection_conservative} in the harmonic
balance framework is given by:
\begin{equation}
	\fbox{$
...
Choosing a uniform sampling for the mono-frequential
formulation as the good property of being well conditioned.
Before going into detail, let us recall what is the condition
number:
