...
gives $2N + 1$ equations: $2N$ perturbation equations and one time
averaged equation. A pseudo-time ($\tau$) derivative is
...
position was equal to $0.8\%$ of efficiency.
...
to a classical time-marching scheme. However the results
...
finally applied to a flat plat and a $2$D and $3$D
...
the same unsteady boundary layer test case of \citet{He1998}), 
on the $2$D DLR compressor stage used by \citet{Chen2001}, 
on a $3$D industrial radial turbine.
Finally
...
NLH computation. They show that approximately $40$ grid points are 
...
results to a classical time-marching simulation and the previous
...
The method is validated on a three-cylinder test case and applied
...
on the turbulent equation (the one equation of \citet{Spalart1992}),
...
for instance is unsteady in the opposite row frame
