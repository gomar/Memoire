...
\subsection{Condition number and contra-rotating open rotor aeroelasticity}
\label{sub:condition_nb_and_cror}
...
studied in this thesis, namely blade flutter, has a vibration frequency that
depends on the structural properties of the blade. In general, these two
frequencies are not equal. This disparity has a salient role on the
condition number value.

To emphasis this, let us consider two independent frequencies $f_1$
and $f_2$ that plays the role here of the blade passing frequency and
the vibration frequency. The condition number of the IDFT matrix $E$
using those two frequencies
and evenly space time instances is shown in Fig.~\ref{fig:disparity}.
The problem being symmetric in $(f_1, f_2)$, so are the results.
Almost a half of the choice of frequencies have an IDFT matrix $E$
whose condition number is superior to ten. However, such
condition number might be too large even for small unsteadinesses
HB computations.
\begin{figure}[htb]
  \centering
  \includegraphics*[width=0.5\textwidth]{disparity.pdf}
  \caption{Condition number as a function of a couple of frequencies $(f_1, f_2)$.}
  \label{fig:disparity}
\end{figure}
