...
further in Sec.~\ref{cha:limitations_condition_number} and an innovative
...
associated to the smallest frequency ($2 \pi / \omega_{min}$) 
...
almost-periodic IDFT matrix is built. This rectangular matrix can be seen
...
Initially, the vector have the same Euclidean norm.
...
The almost-periodic IDFT matrix is then built for
...
involved, five time instances are required. The results of three
...
the number of time instances used, such strategy can not be
used in an industrial context. The APFT
algorithm improves the results, as it gives $\kappa (E) \leq 10$ 
for a large interval but fails when the frequencies
are too close to one another ($y=x$), and when they are significantly
...
for period $1/f_2$. The APFT algorithm is nowhere near the evenly spaced
