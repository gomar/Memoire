...
\begin{figure}[htb]
  ...
  Wakes are depicted with dotted lines.}
...
We consider again the linear advection problem, 
with $u_l$ now taken equal to a Gaussian function of the form Eq.~\eqref{eq:similarity}.
The width $L$ is set to 10\% of the domain size, 
...
Precisely, the Fourier transform $\hat{g}$ of a Gaussian function $g$
defined as:
\begin{equation}
    g(x) = A e^{-\alpha x^2},
...
In the preceding Section, the Gaussian function had a width
