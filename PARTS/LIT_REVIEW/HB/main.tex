...
\chabstract{The main \added{frequency domain}
...
while the three others are built to take into account the 
...
(namely an installed pusher CROR) or a CROR with 
vibrating blades, which is
the purpose of the current work.
The applicability of 
these methods is demonstrated in the literature
through simple test cases for which an analytical
solution is known, $2$D/$3$D academic 
...
is about $2N+1$ times the cost of a steady
computation with $N$ being the number of computed harmonics.
The current study relies on the former work of \citet{ThesisSicot}, who initially
implemented the harmonic balance method into the \textit{elsA}
code at CERFACS, and on the work of \citet{ThesisGuedeney}, who extended 
it to the multi-frequential framework.}
...
\label{sec:sm_intro}
\input{PARTS/LIT_REVIEW/hb/intro}
...
\label{sub:sm_nlh}
\input{PARTS/LIT_REVIEW/HB/nlh}

\section{The \texorpdfstring{\underline{N}}{N}on-\texorpdfstring{\underline{L}}{L}inear 
\texorpdfstring{\underline{F}}{F}requency \texorpdfstring{\underline{D}}{D}omain method (NLFD)}
\label{sub:sm_nlfd}
\input{PARTS/LIT_REVIEW/HB/nlfd}

\section{The \texorpdfstring{\underline{H}}{H}armonic \texorpdfstring{\underline{B}}{B}alance method (HB)}
\label{sec:sm_hb}
\input{PARTS/LIT_REVIEW/HB/hb}

\section{Convergence of the spectral operator}
\label{sec:spectral_accuracy}
\input{PARTS/LIT_REVIEW/HB/convergence}

\section{Periodic flows in turbomachinery}
\label{sec:sm_hudson}
\input{PARTS/LIT_REVIEW/HB/hudson}

\chconclu{Four Fourier-based time methods have been presented in this chapter.
The main mathematical developments have been discussed and 
the underlying hypotheses/weaknesses of each method have been highlighted.
The harmonic balance method, that will be used
in the current work, has been selected because of its ability to
tackle both mono- and multi-frequential applications. To do so, the
...
The vast literature available on these methods shows that
they are ready for industrial, numerically demanding
unsteady applications. Moreover, the method is shown to be suitable for
the computation of aeroelastic phenomena on 
contra-rotating configurations investigated in the present work.
Nevertheless, some numerical issues may arise in specific configurations. 
Before applying the harmonic balance method to CROR, we investigate these aspects 
in the following chapters and suggest possible solutions.}
